...
mod1 = lm(fold_change_transcriptomic~Fold_change_proteomic, data = df1)
mod1
modsum = summary(mod1)
...
ggplot(df1, mapping = aes(x = Fold_change_proteomic, y = fold_change_transcriptomic, color = Status)) +
  geom_point() + geom_text(size = 2, hjust = 0, nudge_y = 0.05, aes(label = Gene_names)) + scale_color_manual(values=c("green", "red")) + stat_smooth(method = "lm", col = "yellow") 
df1
